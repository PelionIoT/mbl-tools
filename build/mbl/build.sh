...
repo_init_atomic ()
{
  ...
  mkdir -p "$path-ri"
  (cd "$path-ri" && repo init --repo-branch="83a3227b62c936b346b825b333fc2ca65528ecfd" --no-repo-verify "$@")
  mv "$path-ri" "$path"
}
...
while true; do
  ...
  sync)
    ...
    (cd "$builddir/mbl-manifest"; repo init --repo-branch="83a3227b62c936b346b825b333fc2ca65528ecfd" --no-repo-verify -b "$branch" -m "$manifest")
  ...
  sync-pinned)
    for machine in $machines; do
      ...
      (cd "$builddir/machine-$machine/mbl-manifest"
       repo init --repo-branch="83a3227b62c936b346b825b333fc2ca65528ecfd" --no-repo-verify -m "pinned-manifest.xml"
       repo sync
      )
    done
