...
class SGlobalFuncs:
    """This class group together short helper functions that are used (or might be used) by multiple objects"""
    ...
    def build_url_from_base_repo_name(remote_prefix, prefix, base_name):
        """"""
        return SGlobalFuncs.build_url_from_repo_name(remote_prefix, prefix + "/" + base_name)
    ...
    def clone_repo(dest_full_path, url, checkout_rev_name="refs/heads/master"):
        """
        clone a repository from 'url' into path 'dest_full_path' and checkout revision 'checkout_rev_name'
        ...
        if (SGlobalFuncs.is_valid_git_branch_name(checkout_rev_name) and
            ...
            raise ValueError(
                "Invalid checkout_rev_name %s to checkout after cloning!" % checkout_rev_name)
        ...
        # now clone
        if is_commit_hash:
            logger.info("Cloning repository {} to {} and checking out commit hash {}".format(
                url, dest_full_path, co_branch))
            ...
            logger.info("Cloning repository {} to {} and checking out branch {}".format(
                url, dest_full_path, co_branch))
...
class CRepoManifestFile(object):
    """This class stores information about manifest XML file in armmbed/mbl-manifest repository """
...
class CRepoManifestProject(object):
    """
    This class represents a google repo manifest file 'project' entry that needs to be cloned
    Each CRepoManifestFile holds one or more CRepoManifestProject objects inside repo_name_to_proj_dict
    """
    ...
    def __init__(self, full_name, prefix, short_name, remote_key, url, revision):
        ...
        # revision to checkout, can be a branch, tag or commit hash (the last 2 are experimental)
        self.revision = revision

        # place holder for CGitClonedRepository object
        self.git_cloned_repository = None
...
class CGitClonedRepository(object):
    ...
    All cloned repositories are kept under CRepoManifestProject::git_cloned_repository or
    CReleaseManager::additional_repo_name_to_git_cloned_repository_dict
    """
    ...
    def __init__(self, remote, name_prefix, short_name, clone_base_path, checkout_rev):
        ...
        if (self.checkout_rev.startswith(REF_BRANCH_PREFIX) or
            ...
            try:
                self.handle = SGlobalFuncs.clone_repo(
                    self.clone_dest_path, self.url, REF_BRANCH_PREFIX + self.checkout_rev)
            except ValueError:
                # try to clone as tag
                self.handle = SGlobalFuncs.clone_repo(
                    self.clone_dest_path, self.url, self.checkout_rev)
        ...
        logger.debug("Created new {} : {}".format(type(self).__name__, pformat(locals())))
        logger.info("{} Cloned from remote {} to folder {}".format(self.full_name, self.url, self.clone_dest_path))
...
class CReleaseManager(object):
    ...
    def __init__(self):
        ...
        """
        Dictionary of CGitClonedRepository objects. Carry only cloned repositories under 
        INPUT_FILE_ADDITIONAL_SD_KEY_NAME section other manifest git repositories are kept per project 
        (CRepoManifestProject:git_cloned_repository object).
        Key - repo name
    ...
    def process_manifest_files(self):
        ...
        logger.info("Cloning repository %s checkout branch %s" % (MBL_MANIFEST_REPO_NAME, self.mbl_manifest_clone_ref))
        ...
        for file_path in xml_file_list:
            ...
            for atype in root.findall('project'):
                ...
                if short_name in name_to_proj_dict:
                    raise ValueError(
                        "File %s : project %s repeats multiple times!".format(file_path, short_name))
    ...
    def validate_remote_repositories_state(self):
        ...
        for file_obj in self.manifest_file_name_to_obj_dict.values():
            ...
            for (k, v) in file_obj.repo_name_to_proj_dict.items():
                ...
                url = SGlobalFuncs.build_url_from_repo_name(
                    file_obj.remote_key_to_remote_dict[v.remote_key], v.full_name)
                new_ref = self.get_new_ref_from_new_revision_dict(
                    file_obj.filename, k)
                if not new_ref:
                    continue
        ...
        logger.info("Starting %d concurrent threads to validate remote repositories state..." % len(check_remote_list))
        with concurrent.futures.ThreadPoolExecutor(max_workers=len(check_remote_list)) as executor:
            future_to_git_url = {
                executor.submit(self.validate_remote_repositories_state_helper, tup[idx_url], tup[idx_rev]):
            ...
            for future in concurrent.futures.as_completed(future_to_git_url, MAX_TMO_SEC):
                ...
                if result:
                    raise argparse.ArgumentTypeError(
                        "revision {} exist on remote url {}".format(tup[idx_rev], tup[idx_url]))
    ...
    def validate_cross_dependencies(self):
        ...
        logger.info("Validating cross dependencies (input file vs manifest files)...")
        ...
        for file_name, sd in self.new_revisions_dict.items():
            ...
            for repo_name in sd:
                ...
                if not found:
                    raise ValueError(
                        "invalid input file : entry ({}, {}) not found!".format(file_name, repo_name))
    ...
    def parse_and_validate_input_file(self):
        ...
        if INPUT_FILE_ADDITIONAL_SD_KEY_NAME not in self.new_revisions_dict:
            raise ValueError("main entry key %s could not be found in user input file" %
                             INPUT_FILE_ADDITIONAL_SD_KEY_NAME)
        if MBL_MANIFEST_REPO_NAME not in self.new_revisions_dict[INPUT_FILE_ADDITIONAL_SD_KEY_NAME]:
        ...
        self.mbl_manifest_clone_ref = self.new_revisions_dict[
            INPUT_FILE_ADDITIONAL_SD_KEY_NAME][MBL_MANIFEST_REPO_NAME][0]
    ...
    def update_mbl_linked_repositories_conf_helper(self, git_repo):
        ...
        logger.info("Start updating file %s in repository %s (if needed)" %
                    (MBL_LINKED_REPOSITORIES_REPO_PATH, MBL_LINKED_REPOSITORIES_REPO_NAME))
    ...
    def update_mbl_linked_repositories_conf(self):
        ...
        if MBL_LINKED_REPOSITORIES_REPO_NAME in self.additional_repo_name_to_git_cloned_repository_dict:
            self.update_mbl_linked_repositories_conf_helper(
                self.additional_repo_name_to_git_cloned_repository_dict[MBL_LINKED_REPOSITORIES_REPO_NAME])
    ...
    def clone_and_create_new_revisions(self):
        ...
        logger.info("Starting %d concurrent threads to clone repositories..." % len(clone_tup_list))
    ...
    def get_argument_parser(self):
        ...
        parser.add_argument(
            ...
            help="path to update.json file which holds a dictionary of pairs (repository name, branch / tag / hash). \
            For more information and exact format see mbl-tools/build-mbl/README.md."
        )
...
def _main():
    ...
    with CReleaseManager() as rm:
        ...
        # Update new_revision for all manifest files projects and create reference (where needed on
