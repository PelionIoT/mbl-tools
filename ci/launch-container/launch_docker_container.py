...
The container images are based on a 'py-deploy-base-image'.
The first step builds the 'py-deploy-base-image',
which is based on ubuntu:bionic-20180724.1.
...
A docker container is then launched, optionally copying any artifacts
back to the workdir.
"""
...
def _main():
    ...
    docker.build(
        "-t", "py-deploy-base-image", "-f", BASE_DOCKERFILE_PATH, SCRIPT_DIR
    )
    # Extend with specific image
    docker.build(
        "-t", cli_args.image, "-f", cli_args.dockerfile, cli_args.workdir
    )
    # Run the container
    ...
    if cli_args.cp:
        docker.cp(
            "{}:{}".format(cli_args.container, cli_args.cp), cli_args.workdir
        )
    # Clean up
